...
public class SAMLEndpoint {
    ...
    protected abstract class Binding {
        ...
        public Response execute(String samlRequest, String samlResponse, String relayState, String clientId) throws IOException {
        ...
        /*            event.event(EventType.IDENTITY_PROVIDER_RESPONSE);
                ...
                if (config.isPassSetMfa() && authn.getAuthnContext() != null && authn.getAuthnContext().getSequence() != null && authn.getAuthnContext().getSequence().getClassRef() != null ) {
                    Integer idpLoa = AcrUtils.getAcrLoaMap(authSession.getClient()).get(authn.getAuthnContext().getSequence().getClassRef().getValue().toString());
        ...
        private String getExpectedDestination() {
            return Urls.samlFederationAuthnResponse(session.getContext().getUri().getBaseUri(), realm.getName()).toString();
        }
    }
